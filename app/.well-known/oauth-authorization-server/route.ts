...
import { ajDiscovery } from "@/lib/arcjet-config";
import { logger } from "@/lib/logger";
import { type NextRequest } from "next/server";

export async function GET(req: NextRequest) {
  // Arcjet protection for discovery endpoint
  const decision = await ajDiscovery.protect(req, { requested: 1 });
  ...
  if (decision.isDenied() && decision.reason.isRateLimit()) {
    logger.security("Rate limit exceeded on discovery endpoint", {
      ip: clientIp,
    }, "info");
  ...
  logger.debug("Authorization server metadata requested", { ip: clientIp });
